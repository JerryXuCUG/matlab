...
class = ["_sand.mat","_mud.mat","_baiyunyan.mat"];
svm_model_sand = load(Model(1) + class(1));
pnn_model_sand = load(Model(2) + class(1));
svm_model_mud = load(Model(1) + class(2));
pnn_model_mud = load(Model(2) + class(2));
svm_model_dolomite = load(Model(1) + class(3));
pnn_model_dolomite = load(Model(2) + class(3));
...
for i = 1:size(data_std,1)
    ...
    if contains(temp_class,"砂岩") || (contains(temp_class,"砂砾岩"))
        temp_data_sand = [temp_data_sand; [temp_data,i]];
    elseif contains(temp_class,"泥岩")
        temp_data_mud = [temp_data_mud; [temp_data,i]];
    elseif contains(temp_class,"白云岩")
        temp_data_dolomite = [temp_data_dolomite; [temp_data,i]];
    else
...
% 砂岩段
if size(temp_data_sand, 1) >= 1
    [predict_label,socre] = predict(svm_model_sand.SVMmodel, temp_data_sand(:,1:length(param_index)));
    for i = 1:length(predict_label)
        index = temp_data_sand(i,length(param_index)+1);
        predict_res(index,1) = predict_label(i);
        predict_res(index,2) = max(socre(i,:));
    end
    predict_label = sim(pnn_model_sand.net_pnn, temp_data_sand(:,1:length(param_index))');
...
% 白云岩段
if size(temp_data_dolomite, 1) >= 1
    [predict_label,socre] = predict(svm_model_dolomite.SVMmodel, temp_data_dolomite(:,1:length(param_index)));
    for i = 1:length(predict_label)
        index = temp_data_dolomite(i,length(param_index)+1);
        predict_res(index,1) = predict_label(i);
        predict_res(index,2) = max(socre(i,:));
    end
    predict_label = sim(pnn_model_dolomite.net_pnn, temp_data_dolomite(:,1:length(param_index))');
    predict_label = vec2ind(predict_label);
    for i = 1:length(predict_label)
        index = temp_data_dolomite(i,length(param_index)+1);
        if predict_label == 2
...
clear temp_class temp_data_dolomite temp_data_mud temp_data_sand;
